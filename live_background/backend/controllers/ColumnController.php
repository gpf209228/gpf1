...
class ColumnController extends SumController
{
	...
	public function actionAdd()
	{
		...
		if($data = Yii::$app->request->post()){
           ...
           if(!empty($_POST['class_name'])){
               $sql=$db->createCommand("select * from column where class_name='$_POST[class_name]'")->execute();
                if(!$sql){
                   $sql= $db->createCommand("insert into column (`class_rank`,`class_name`,`img`,
            ...
            $sql="select * from column where class_p_id='0'";
	...
	public function actionList()
	{
        if($data = Yii::$app->request->post()){
            ...
            $data = Column::find()->asArray();
            $pages = new Pagination(['totalCount' =>$data->count(), 'pageSize' => '4']);
            $sql = $data->offset($pages->offset)->limit($pages->limit)->all();
            $sql=$this->subtree($sql);
            return $this->render('list',array('class'=>$sql, 'pages' => $pages));
        }
	}
	...
	public function actionSave()
	{
        if($data = Yii::$app->request->post()){
            ...
            if(isset($_POST['class_id'])){
                $db=yii::$app->db;
                    $sql="update column set class_rank='$_POST[class_rank]',class_name='$_POST[class_name]',class_p_id='$_POST[class_p_id]' where class_id='$_POST[class_id]'";
                ...
                if(!$_FILES['file']['name']==""){
                    if ((($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/pjpeg")) && ($_FILES["file"]["size"] < 200000))
                    {
                        if ($_FILES["file"]["error"] ==0)
                        {
                            ...
                            $sqls="update column set class_img='$_POST[class_img]',class_path='$_POST[class_path]' where class_id='$_POST[class_id]'";
            ...
            $db=yii::$app->db;
            $sqls="select * from column where class_p_id=0";
            $sqls=$db->createCommand($sqls)->queryAll();
            $sql="select * from column where class_id='$_GET[class_id]'";
	...
	{
        $db=yii::$app->db;
        $sql="delete from column where class_id='$_GET[class_id]'";
